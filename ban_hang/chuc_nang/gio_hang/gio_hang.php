...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  //kiểm tra xem tên có bị rỗng hay không
  if (empty($_POST['so_luong'])) {
    $slMessage = 'Trường tên không được bỏ trống.';
  } else {
    $name = htmlspecialchars($_POST['so_luong']);}}
		...
		echo "<table>";
			...
			for($i=0;$i<count($_SESSION['id_them_vao_gio']);$i++)
			{
				...
				echo "<tr>";
					...
					echo "<input type='number' style='width:50px' min='0' ".$name_sl."' value='". $_SESSION['sl_them_vao_gio'][$i]."' > ";
					echo "</td>";
					echo "<td>".$tv_2['gia']."</td>";
					echo "<td>".$tien."</td>";
				echo "</tr>";
